...
import { LoginButton, LoginManager, ShareDialog, AccessToken, GraphRequest, GraphRequestManager } from 'react-native-fbsdk';
...
export default class App extends React.Component {
  ...
  GoogleUploadToDB(name, image) {
    firebase.database().ref('GUsers/').set({
      Username: name,
      Userimage: image
    }).then((data) => {
      alert('data ', data)
  ...
  onLoginOrRegister = async () => {
    ...
    GoogleSignin.signIn()
      ...
      .then((userInfo) => {
        ...
        this.GoogleUploadToDB(this.state.name, this.state.image)
      })
      .catch((error) => {
      });
  }
